	...
	if (finfo != 0) {
		...
		if (p[0]==0x4d && p[1]==0x5a) {
			...
			*((int *) (p + 0x0020)) = HeapAddress;
			...
			int j,SectionSize,Address,VirtualAddress;
			for (j = 0; j < NumberofSections-2; j++) {
				SectionSize = *((int *) (p + 0x01a8 + j*0x28));
				VirtualAddress = *((int *) (p + 0x01ac + j*0x28)) + ImageBase;
				Address = *((int *) (p + 0x01b4 + j*0x28));
				for (i = 0; i < SectionSize; i++) {
					q[VirtualAddress + i] = p[Address + i];
				}
			}
			start_app(codhrb, 0 * 8 + 4, esp, 1 * 8 + 4, &(task->tss.esp0));
